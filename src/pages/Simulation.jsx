...
export function Simulation() {
  ...
  const loadLeagues = async () => {
    try {
      ...
      setLeagues((result.items || result).filter(l => l.status === 'Activa'));
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold text-gray-800 dark:text-slate-100">Simulación de Partidos</h1>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Configuración */}
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6 border border-gray-200 dark:border-slate-700">
          <h2 className="text-lg font-bold text-gray-800 dark:text-slate-100 mb-4">Configurar Simulación</h2>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Equipo Local *</label>
                <select
                  value={homeTeamId}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Equipo Visitante *</label>
                <select
                  value={awayTeamId}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Jornada</label>
                <input
                  type="number"
                  min="1"
                  value={round}
            ...
            <div className="pt-4 border-t border-gray-200 dark:border-slate-600">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-medium text-gray-700 dark:text-slate-300">Probabilidades</h3>
                <span className={`text-sm font-bold ${
                  homeWinProb + drawProb + awayWinProb === 100 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                }`}>
            ...
            <button
              ...
              className="w-full bg-green-600 dark:bg-green-500 text-white py-3 px-4 rounded-md hover:bg-green-700 dark:hover:bg-green-600 transition-colors font-bold text-lg disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <div className="space-y-6">
          {result ? (
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6 text-center border border-gray-200 dark:border-slate-700">
              <span className="text-6xl">🎮</span>
              <p className="text-gray-500 dark:text-slate-400 mt-4">Configura las probabilidades y simula un partido</p>
              <p className="text-gray-400 dark:text-slate-500 text-sm mt-2">El resultado se mostrará aquí</p>
            </div>
          )}
          ...
          {stats && stats.totalSimulations > 0 && (
            <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6 border border-gray-200 dark:border-slate-700">
              <h3 className="text-lg font-bold text-gray-800 dark:text-slate-100 mb-4">Estadísticas</h3>
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
                  <p className="text-2xl font-bold text-gray-800 dark:text-slate-100">{stats.totalSimulations}</p>
                  <p className="text-xs text-gray-500 dark:text-slate-400">Total Simulaciones</p>
                </div>
                <div className="text-center p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
                  <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{stats.avgHomeGoals}</p>
                  <p className="text-xs text-gray-500 dark:text-slate-400">Prom. Goles Local</p>
                </div>
                <div className="text-center p-3 bg-gray-50 dark:bg-slate-700 rounded-lg">
                  <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{stats.avgAwayGoals}</p>
                  <p className="text-xs text-gray-500 dark:text-slate-400">Prom. Goles Visitante</p>
                </div>
              </div>
            </div>
          )}
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-gray-800 dark:text-slate-100">Historial</h2>
        <button
